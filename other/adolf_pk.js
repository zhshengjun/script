/*
京享值pk:脚本更新地址 https://github.com/inoyna11/jd28/blob/master/adolf_pk.js
更新时间：2021-6-4
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
==========================Quantumultx=========================
[task_local]
#京享值pk
0 6-18/4 * * * https://github.com/inoyna11/jd28/blob/master/adolf_pk.js, tag=京享值pk, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
=========================Loon=============================
[Script]
cron "0 6-18/4 * * *" script-path=https://github.com/inoyna11/jd28/blob/master/adolf_pk.js, tag=京享值pk

=========================Surge============================
京享值pk = type=cron,cronexp="0 6-18/4 * * *",wake-system=1,timeout=3600,script-path=https://github.com/inoyna11/jd28/blob/master/adolf_pk.js, tag=京享值pk

=========================小火箭===========================
京享值pk = type=cron,script-path=https://github.com/inoyna11/jd28/blob/master/adolf_pk.js, cronexpr="0 6-18/4 * * *", timeout=3600, enable=true

京享值pk 搬的https://github.com/inoyna11/jd28/blob/master/adolf_pk.js
*/
...
let authorPin='66671c54212be6197e0b2f240382eb50';
$.helpAuthor=true;
!(async () => {
	$.log('wobuxin')
	if (!cookiesArr[0]) {
	...
	if ($.token) {
		...
		if($.pinList){
		    for(let index=0;index<$.pinList.length;index++){
		        ...
		        if(fscore<myScore){
					await getMyRankInfo($.pin);
					if($.myInfo){
						if($.myInfo.leftLunchPkNum===0){
							break;
						}
					}
		            await launchBattle(pin);
	            	await receiveBattle(pin);
					await launchBattle($.pin,pin);
	            	await receiveBattle($.pin,pin);
		        }
		    }
		}
		if($.helpAuthor){
			await getMyRankInfo(authorPin);
			if($.myInfo){
				if($.myInfo.leftAcceptPkNum!=null||$.myInfo.leftAcceptPkNum!='null'){
...
function getPinList(fpin){
    //https://api.r2ray.com/jd.pk/index
    that.log("获取Pk列表");
	return new Promise((resolve) => {
		let options = {
			"url": "https://pengyougou.m.jd.com/like/jxz/getUserFriends?actId=8&appId=dafbe42d5bff9d82298e5230eb8c3f79&lkEPin="+fpin,
			"headers": {
...
function getMyRankInfo(fpin){
    //https://api.r2ray.com/jd.pk/index
    that.log("获取我的pk次数");
	return new Promise((resolve) => {
		let options = {
			"url": "https://pengyougou.m.jd.com/like/jxz/getMyRankInfo?actId=8&appId=dafbe42d5bff9d82298e5230eb8c3f79&lkEPin="+fpin,
			"headers": {
				"Host": "pengyougou.m.jd.com",
				"Content-Type": "application/json",
				"Origin": "https://game-cdn.moxigame.cn",
...
function launchBattle(fpin,mypin) {
	if(!mypin){
		mypin = $.pin;
	}
	that.log("发起挑战");
	return new Promise((resolve) => {
		let options = {
			"url": `https://jd.moxigame.cn/likejxz/launchBattle?actId=8&appId=dafbe42d5bff9d82298e5230eb8c3f79&lkEPin=${mypin}&recipient=${fpin}&relation=1`,
			"headers": {
				"Host": "jd.moxigame.cn",
				"Content-Type": "application/json",
				"Origin": "https://game-cdn.moxigame.cn",
...
function receiveBattle(fpin,mypin) {
	if(!mypin){
		mypin = $.pin;
	}
	return new Promise((resolve) => {
		let options = {
			"url": `https://jd.moxigame.cn/likejxz/receiveBattle?actId=8&appId=dafbe42d5bff9d82298e5230eb8c3f79&lkEPin=${mypin}&recipient=${fpin}`,
			"headers": {
				"Host": "jd.moxigame.cn",
				"Content-Type": "application/json",
				"Origin": "https://game-cdn.moxigame.cn",
